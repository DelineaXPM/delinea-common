...
// name/value pairs with unique names. It is the engine behind the
// "delinea-util secrets" subcommand group and can be embedded directly by Go CI
// integrations.
...
// Resolve fetches every mapping and returns the resulting variables in order.
// Variable names must be unique across the whole batch, including names
// generated by expansion. A secret referenced by multiple mappings is fetched
...
func duplicateVariableError(name string) error {
	return fmt.Errorf("variable %q is defined more than once", name)
}

func claimVariableName(seen map[string]struct{}, name string) error {
	if _, duplicate := seen[name]; duplicate {
		return duplicateVariableError(name)
	}
	seen[name] = struct{}{}
	return nil
}

// claimVariableNames is atomic: when one name collides, none of the names from
// that expansion are reserved. Verify can therefore continue with later
// mappings without a failed mapping affecting their results.
func claimVariableNames(seen map[string]struct{}, vars []Var) error {
	pending := make(map[string]struct{}, len(vars))
	for _, v := range vars {
		if _, duplicate := seen[v.Name]; duplicate {
			return duplicateVariableError(v.Name)
		}
		if _, duplicate := pending[v.Name]; duplicate {
			return duplicateVariableError(v.Name)
		}
...
func (c *Client) resolve(ctx context.Context, mappings []Mapping) ([]Var, error) {
	...
	for _, m := range mappings {
		...
		if !m.Expand {
			if err := claimVariableName(directNames, m.EnvName); err != nil {
				return nil, err
			}
		}
	}
	cache := map[string]*Secret{}
	claimedNames := map[string]struct{}{}
	var out []Var
	for _, m := range mappings {
		if !m.Expand {
			// Direct names are known before fetching. Claim them first so a
			// collision with an earlier expansion cannot spend another credential.
			if err := claimVariableName(claimedNames, m.EnvName); err != nil {
				return nil, err
			}
		}
		...
		if m.Expand {
			if err := claimVariableNames(claimedNames, vars); err != nil {
				return nil, fmt.Errorf("secret %s: %w", m.Ref(), err)
			}
		}
		out = append(out, vars...)
	}
...
// references can be corrected in one pass. It also reports a later mapping
// whose variable name collides with an earlier structurally valid mapping. It
// returns names and value lengths, never values. The returned error is non-nil
// only if the whole call timed out or ctx was cancelled; per-mapping failures
// are in Result.Err.
...
func (c *Client) verify(ctx context.Context, mappings []Mapping) ([]Result, error) {
	...
	for _, m := range mappings {
		...
		if m.Expand {
			if err := claimVariableNames(claimedNames, vars); err != nil {
				r.Err = fmt.Errorf("secret %s: %w", m.Ref(), err)
				out = append(out, r)
				continue
			}
		}
